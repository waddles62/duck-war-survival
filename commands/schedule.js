    ...
    if (sub === 'post') {
      ...
      collector.on('collect', async (reaction, user) => {
        try {
          ...
          const todayName = SHORT_DAYS[today];
          const tomorrowName = SHORT_DAYS[tomorrow];

          const todayLines = [];
          if (byDay[today] && byDay[today].length > 0) {
            byDay[today].forEach(item => {
              const time = formatTime(item.utcHour, item.utcMinute, tz);
              todayLines.push(`${time}  ${item.name}`);
            });
          } else {
            todayLines.push('No events scheduled');
          }
